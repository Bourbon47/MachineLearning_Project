...
X_train = df_DF_train.iloc[:,1:(len(df_DF_train.columns)-1)]
y_train = df_DF_train.iloc[:,(len(df_DF_test.columns)-1)]

X_test = df_DF_test.iloc[:,1:(len(df_DF_train.columns)-1)]
y_test = df_DF_test.iloc[:,(len(df_DF_test.columns)-1)]
...
Log_reg = LogisticRegression(penalty= penalty , C = C , solver = 'saga', random_state=10)
